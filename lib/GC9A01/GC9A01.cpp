...
SPI.begin(TFT_SCLK, TFT_MISO, TFT_MOSI, TFT_CS); // (SCLK, MISO, MOSI, SS)

// Set the SPI frequency and mode (Mode 0 is common for displays)
SPI.beginTransaction(SPISettings(SPI_FREQUENCY, MSBFIRST, SPI_MODE0));
*/

#define DISPLAY_SCLK 14 // Custom SCLK pin
#define DISPLAY_MOSI 15 // Custom MOSI pin
#define DISPLAY_MISO -1 // Not used, set to -1 if not needed
#define DISPLAY_CS   13  // Chip Select pin for the LCD
#define DISPLAY_DC   12  // Data/Command pin
#define DISPLAY_RST  16  // Reset pin (optional)


// SPI settings
#define SPI_FREQUENCY 10000000 // 40 MHz (adjust according to your display's capabilities)
...
// SPI settings
#define SPI_FREQUENCY 27000000 // 40 MHz (adjust according to your display's capabilities)

void LV_DRV_DISP_SPI_CS(uint8_t state) {
    digitalWrite(DISPLAY_CS, state);
}

void LV_DRV_DISP_RST(uint8_t state) {
    digitalWrite(DISPLAY_RST, state);
}

void LV_DRV_DISP_CMD_DATA(uint8_t state) {
    digitalWrite(DISPLAY_DC, state);
}


void LV_DRV_DISP_SPI_WR_BYTE(uint8_t byte) {
    SPI.transfer(byte);
}

void LV_DRV_DISP_SPI_WR_ARRAY(const char *data, size_t len) {
    SPI.writeBytes((const uint8_t *)data, len);
}


void LV_DRV_DELAY_MS(uint16_t ms) {
    delay(ms);
}
...
// GC9A01 command functions
static void GC9A01_command(uint8_t cmd) {
    LV_DRV_DISP_CMD_DATA(0);  // Command mode
...
/**
 * Initialize the GC9A01GC9A01_fillScreen
 */
int GC9A01_init(void)
{
	...
	GC9A01_fillRect(50, 50,  GC9A01_HOR_RES, GC9A01_VER_RES, 0xFFFF);

	delay(1000);

	GC9A01_fillRect(150, 150,  GC9A01_HOR_RES, GC9A01_VER_RES, 0xFFFF);
...
void GC9A01_flush(lv_display_t *disp_drv, const lv_area_t *area, uint8_t *color_p) {
    LV_DRV_DISP_SPI_CS(0); // Activate SPI chip select

    // Set the display window to update
    GC9A01_set_addr_win(area->x1, area->y1, area->x2, area->y2);

    // Calculate the number of pixels
    int32_t len = (area->x2 - area->x1 + 1) * (area->y2 - area->y1 + 1);

    // Write the color buffer to the display
    LV_DRV_DISP_CMD_DATA(GC9A01_DATA_MODE);
    LV_DRV_DISP_SPI_WR_ARRAY((char *)color_p, len * 2); // Multiply by 2 for RGB565 (16-bit per pixel)

    LV_DRV_DISP_SPI_CS(1); // Deactivate SPI chip select
