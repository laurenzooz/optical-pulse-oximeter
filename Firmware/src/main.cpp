...
void setup() {
    ...
    pinMode(4, INPUT);   
    peakDetection.begin(64, 6, 0.8);  // test different values (lag, threshold, influence)
...
void loop() {
    ...
    int mapped_signal = map(hr_signal, average - 200, average + 200, 0, 100);
    ...
    snprintf(label_buffer, sizeof(label_buffer), "%d", hr_signal);
